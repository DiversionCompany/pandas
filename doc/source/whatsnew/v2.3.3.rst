...
- Fixed regression in :meth:`Index.difference` returning too many values for :class:`PeriodIndex` when the subtracted range covers trailing elements of the original index (:issue:`58971`)
...
- Fixed :meth:`DataFrame.loc` incorrectly assigning all ``NaT`` values when setting a datetime column via partial :class:`MultiIndex` key with a simple-indexed :class:`Series` (:issue:`43351`)
- Fixed :func:`testing.assert_frame_equal` (and :func:`testing.assert_series_equal`) incorrectly treating a Python list and a numpy array with the same values as not equal when ``check_exact=True`` was specified (:issue:`63904`)
- Fixed :func:`read_excel` with ``dtype_backend='pyarrow'`` raising ``ArrowInvalid`` for columns with mixed types (e.g. integers and strings); they are now coerced to string type (:issue:`63830`)
