...
- :meth:`Series.plot` and :meth:`DataFrame.plot` now support nullable boolean :class:`ExtensionArray` dtypes (``boolean``) by converting them to float arrays for plotting (:issue:`64300`)
...
- Performance improvement in :func:`tseries.frequencies.to_offset` by replacing O(n) linear scan of deprecated frequency string values with O(1) frozenset lookup (:issue:`58962`)
...
- Bug in :func:`infer_freq` inferring incorrect anchor month for multi-quarterly frequencies (e.g. ``2BQS-OCT`` instead of ``2BQS-JAN``) (:issue:`63791`)
...
- Bug in ``test_dt64arr_cmp_arraylike_invalid`` that failed on Windows with ``tz=tzlocal()`` when the local timezone has a negative UTC offset; the test used nanosecond timestamps near the Unix epoch that caused ``time.localtime`` to fail with a negative timestamp (:issue:`64281`)
- Bug in :meth:`DataFrame.set_index` where the ``freq`` attribute of a :class:`DatetimeIndex` or :class:`TimedeltaIndex` was lost when setting the index from a datetime/timedelta column (:issue:`42747`)
...
- Bug in :meth:`DataFrame.loc` where setting a :class:`Categorical` (or other typed array) into an existing ``object``-dtype column via ``df.loc[:, col] = value`` silently ignored the new dtype instead of converting the column (:issue:`52593`)
...
- Bug in :meth:`DataFrame.plot.bar` and :meth:`DataFrame.plot.barh` raising ``ValueError: x must be a label or position`` when ``x`` is a list of column labels (:issue:`21386`)
...
- Bug in :meth:`.ExponentialMovingWindow.aggregate` (and its alias ``.agg``) raising ``AttributeError`` when passing a single callable; now the callable is invoked on the window object directly (:issue:`63855`)
...
- Bug in :meth:`DataFrame.unstack` with ``sort=False`` producing wrong column labels when the MultiIndex has unused levels (:issue:`64150`)
...
- Bug in ``test_legacy_files`` that raised ``Empty parameter set`` instead of skipping when legacy HDF5 files are not present (e.g. when running tests against an installed wheel without legacy data files) (:issue:`64604`)
