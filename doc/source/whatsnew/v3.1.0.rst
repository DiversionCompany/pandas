...
- Bug in :meth:`DataFrame.loc` where assigning a 2D :class:`numpy.ndarray` to multiple columns via ``df.loc[:, cols] = arr`` would raise :exc:`TypeError` when the array dtype was incompatible with the column dtype (e.g. assigning float values to integer columns), while the equivalent ``df[cols] = arr`` succeeded (:issue:`46544`)
